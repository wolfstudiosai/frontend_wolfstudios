    ...
    return (
      <Box width={220} bgcolor="background.paper" px={1.5}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography fontWeight={500} sx={{ fontSize: '14px', color: 'text.secondary' }}>
            My Views
          </Typography>
        ...
        {personalViews?.length > 0 && (
          <>
            <Box>
              <Typography fontWeight={500} sx={{ fontSize: '14px', color: 'text.secondary', mb: 1 }}>
                Personal Views
              </Typography>
              <Stack>
      ...
      {showView &&
        (isLargeScreen ? (
          <Box width={220} bgcolor="background.paper">
            {viewsLoading ? <CampaignListViewSkelton /> : renderViewSidebar()}
          </Box>
        ) : (
          <Drawer
            anchor="left"
              ...
              <Form>
                ...
                <Box sx={{ p: 1.5 }}>
                  ...
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 2, fontSize: '0.8rem', color: '#666' }}>
                    {values.editPermission === 'collaborative' && <>All collaborators can edit the configuration</>}
                    {values.editPermission === 'personal' && <>Only you can edit the configuration</>}
                    {values.editPermission === 'locked' && <>No one can edit the configuration</>}
                  </Typography>
                </Box>
      ...
      {isRenaming ? (
        ...
        <Box
          ...
          sx={{
            ...
            '&:hover': {
              bgcolor: selectedView?.meta?.id === view.id ? alpha(theme.palette.primary.main, 0.2) : 'action.hover',
            },
