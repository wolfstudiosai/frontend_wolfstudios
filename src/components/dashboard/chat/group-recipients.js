...
//         const response = await api.get(`threads/contacts?query=${encodeURIComponent(query)}`);
...
//         const results = result.data.filter((contact) => {
//           // Filter already picked recipients
//           if (recipients.find((recipient) => recipient.id === contact.id)) {
//             return false;
//           }
  ...
  const fetchContacts = React.useCallback(async (query) => {
    ...
    try {
      ...
      const results = response.data.data.filter(
        (contact) => !recipients.some((r) => r.id === contact.id)
      );
                ...
                {loading ? (
                  ...
                  <React.Fragment>
                    ...
                    <List sx={{ p: 1, '& .MuiListItemButton-root': { borderRadius: 1 } }}>
                      {searchResults.map((contact) => (
                        <ListItem disablePadding key={contact.id}>
                          <ListItemButton onClick={() => handleSearchSelect(contact)}>
                            <ListItemAvatar>
                              <Avatar src={contact.avatar} sx={{ '--Avatar-size': '32px' }} />
                            </ListItemAvatar>
                            <ListItemText
