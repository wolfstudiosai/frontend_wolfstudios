...
export const AddMemberToChannel = (props) => {
  ...
  // set dynamic channel name
  useEffect(() => {
    if (workspaceInfo) {
      setFieldValue('channel', 'channel name');
    }
    ...
    <Dialog title="Add Member" onClose={onClose} open={open}>
      <form onSubmit={handleSubmit}>
        <Grid container spacing={2}>
          <Grid size={{ xs: 12 }}>
            <CustomTextField name="channel" label="Channel" value={values.channel} onChange={handleChange} disabled />
