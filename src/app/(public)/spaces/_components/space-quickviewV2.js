  ...
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      >
        ...
        <Stack direction="column" spacing={1} mb={2}>
            ...
            <Stack direction={'row'} spacing={0.8} mt={0.5}>
              {data?.tags?.length > 0 ? (
                data.tags.map((item, index) => (
                  <CustomChip key={index} label={item?.name || 'Unknown'} color="primary" />
                ))
              ) : (
          ...
          <Stack direction="row" spacing={1}>
            ...
            <Typography variant="subtitle1" color="text.secondary">
              {data?.states?.map((item) => item?.name)?.join(', ') || 'N/A'}
            </Typography>
          </Stack>
          ...
          <Stack direction="row" spacing={1}>
            ...
            <Typography variant="subtitle1" color="text.secondary">
              {data?.cities?.map((item) => item?.name)?.join(', ') || 'N/A'}
            </Typography>
          </Stack>
        </Stack>
      ...
      <SliderWrapper
        ...
        spaceBetween={1.2}
      >
        {data?.showcaseYourLastProjectHereWithUs?.map((item, index) => (
