...
import { Box, Button, Card, CardContent, CardMedia, Divider, IconButton, Stack, Typography } from '@mui/material';
...
export const ProfileSlider = () => {
  ...
  const slides = slider_data.map((item, index) => ({
    id: item.id,
    content: <ProfileSliderItem item={item} index={index} currentIndex={currentIndex} />,
  }));
...
const ProfileSliderItem = ({ item, index, currentIndex }) => {
  return (
    <Card
      sx={{
        width: '100%',
