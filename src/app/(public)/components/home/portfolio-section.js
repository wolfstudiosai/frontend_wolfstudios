      ...
      <Grid container spacing={4}>
        ...
        >
          ...
          <Stack direction='row' sx={{ mt: 4 }}>
            <Button variant='outlined' onClick={() => router.push('/portfolio')}>See all portfolios</Button>
          </Stack>
        </Grid>
        {
          portfolios?.length > 0 && (
              ...
              >
                {
                  portfolios.slice(0, portfolios.length / 2).map((portfolio, index) => (
                    <SwiperSlide key={index}>
                      <PortfolioCard item={portfolio} fetchList={fetchPortfolios} sx={{ borderRadius: 0 }} infoSx={{ width: '86%' }} />
                    </SwiperSlide>
                  ))
                }
              </SliderWrapper>
